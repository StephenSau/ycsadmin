...
app.controller('articleEditCtrl', ['$scope', 'ycsUtil', 'ycsApi', 'Notification', '$modal', '$state', '$filter', '$sce', function (sc, ycsUtil, ycsApi, Notification, $modal, $state, $filter, $sce) {
	sc.sourceListOpts = [];
  ...
  sc.getExistingData = function getExistingData(code){
    ...
    // Is edit
      ...
      var ajaxUrl = 'admin/article/getDetail';
      var qData = {
        id: sc.articleId
      };
  ...
  sc.changeStatus = function changeStatus(num, detail){
    var ajaxUrl = 'admin/article/updateArticle';
  ...
  sc.save = function save(detail, backToList){
    ...
    if( sc.articleId === 'new'){
      ...
      ajaxUrl = 'admin/article/updateArticle';
      qData.id = Number(sc.articleId);
