...
app.controller('requirementCtrl', ['$scope', 'ycsUtil', 'ycsApi', 'Notification', '$modal', function (sc, ycsUtil, ycsApi, Notification, $modal) {
	sc.loadingGrid = true;
  ...
  sc.openModal = function openModal (id, func, data) {
	  var modalInstance = $modal.open({
	    ...
	    size: 'lg',
	    controller: 'requirementModalCtrl',
      backdrop: 'static',
      resolve: {
        predefInfo: function() {
          return {
...
// Modal: 需满足条件
app.controller('requirementModalCtrl', ['$scope', 'predefInfo', '$modalInstance', 'ycsApi', 'Notification', function(sc, predefInfo, $modalInstance, ycsApi, Notification){
	sc.predefInfo = predefInfo;

	sc.isEdit = false;
	sc.title = 'requirement';
